...
<body>
        ...
        <fieldset>
            ...
            <select name="mpg">
              <option value="" NULL 
                ...
                ?>>Select one!</option>  
              <option value="10" 
                <?php if(isset($_POST['mpg']) && $_POST['mpg'] == 10)
                    echo 'selected="selected"';    
                ?>>Bad @ 10mpg</option>  
              <option value="20" 
                <?php if(isset($_POST['mpg']) && $_POST['mpg'] == 20)
                    echo 'selected="selected"';    
                ?>>Alright @ 20mpg</option> 
              <option value="30" 
                <?php if(isset($_POST['mpg']) && $_POST['mpg'] == 30)
                    echo 'selected="selected"';    
                ?>>Good @ 30mpg</option> 
              <option value="40" 
                <?php if(isset($_POST['mpg']) && $_POST['mpg'] == 40)
                    echo 'selected="selected"';    
                ?>>Great @ 40mpg</option> 
            </select>
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $_POST['mpg'])){
            ...
            $total_gas = 0;
            $dollars = 0;
            if(!empty($speed && $hours && $mpg)){
                $total_hours = $miles / $speed;
                $days = $total_hours / $hours;
                $total_gas = $miles / $mpg;
                $dollars = $price * $total_gas;
            }

            if(!empty($name && $total_hours && $days && $total_gas && $dollars)){
                echo '
                <div class="box">
                    <h2>Hello '.$name.', </h2>
                    <p>You will be travelling a total of <b>'.number_format($total_hours, 2).' hours</b>, 
                    taking <b>'.number_format($days, 2).' days.</b> You will be using <b>
                    '.number_format($total_gas, 2).' gallons</b> of gas, costing you 
                    <b>$'.number_format($dollars, 2).'</b></p>
                </div>
                ';
            }
        }
